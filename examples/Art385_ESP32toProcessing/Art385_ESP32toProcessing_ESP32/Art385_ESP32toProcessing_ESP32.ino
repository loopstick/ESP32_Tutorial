/*
    Code modified frorm the example:
  ...
  The circuit: sensors attached to analog inputs 0, 1, and 2
  
    For our ART 385 platform we have 
    Button for the Red
    Potentiometer for Green
    nothing for Blue

  Upload this ESP32 code, then copy the code below into a processing sketch and run it
  You may need to adjust the myPort setting to get Processing to see your ESP32.
...
const int redPin = A0;      // sensor to control red color - Button
const int greenPin = A1;    // sensor to control green color - potentiometer
const int bluePin = A2;     // sensor to control blue color - nothing :(
...
void loop() {
  ...
  redVal = map(redVal, 0, 4095, 0, 255);
  greenVal = map(greenVal, 0, 4095, 0, 255);
  blueVal = map(blueVal, 0, 4095, 0, 255);

  // send Serial message to Processing
  Serial.print(redVal);
  Serial.print(",");
